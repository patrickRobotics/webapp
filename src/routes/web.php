...
Route::get('post/{id}', function ($id) {
    if ($id == 1) {
        $post = [
            ...
            'content' => 'Here is where you can register web routes for your application. These
            routes are loaded by the RouteServiceProvider within a group which
            contains the "web" middleware group. Now create something great!'
        ];
    } else {
        $post = [
            ...
            'content' => 'Some other content on how to create something great!'
        ];
    }
...
Route::group(['prefix' => 'admin'], function() {
    ...
    Route::get('edit/{id}', function ($id) {
        if ($id == 1) {
            $post = [
                'title' => 'Learning Laravel',
                'content' => 'Laravel create something great!'
            ];
        } else {
            $post = [
                'title' => 'Something Else',
    ...
    Route::post('edit', function(\Illuminate\Http\Request $request,	Illuminate\Validation\Factory $validator) {
        ...
        return redirect()
        ->route('admin.index')
        ->with('info', 'Post edidted, new Title: '. $request->input('title'));
